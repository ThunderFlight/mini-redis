...
impl Client {
    ...
    /// ```
    #[instrument(skip(self))]
    pub async fn get(&mut self, key: &str, handler: String) -> crate::Result<Option<Bytes>> {
    ...
    /// ```
    #[instrument(skip(self))]
    pub async fn set(&mut self, key: &str, value: Bytes, handler: String) -> crate::Result<()> {
        ...
        self.set_cmd(Set::new(key, value, handler, None)).await
    }
    ...
    pub async fn set_expires(
        ...
        // used to set a value with an expiration. The common parts of both
        // functions are implemented by `set_cmd`.
        self.set_cmd(Set::new(key, value, handler, Some(expiration)))
            .await
    }
